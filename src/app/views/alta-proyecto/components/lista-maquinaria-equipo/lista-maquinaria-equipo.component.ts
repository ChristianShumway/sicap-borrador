...
export class ListaMaquinariaEquipoComponent implements OnInit {
  ...
  verifyPersonalExist() {
    this.listaMaquinariaEquipoService.getListObservable(this.idObra);
  ...
  initUploadCatalogo() {
    ...
    this.uploaderListaMaqEquipo = new FileUploader({ url: this.rutaServe + '/obra/uploadMaterial', autoUpload: true, headers: headers });
  ...
  eliminarCatalogo() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result){
        this.listaMaquinariaEquipoService.removeListaMaquinariaEquipo(this.idObra).subscribe(
          response => {
