...
export class ModalLlenarPeticionComponent implements OnInit {
  ...
  getValidations(){
    this.peticionForm = new FormGroup({
      ...
      cantidad: new FormControl(this.data.peticion.cantidad),
      cantidadSuministrto: new FormControl(this.data.peticion.cantidadSuministrto, Validators.required),
  ...
  llenarPeticion(){
    if(this.peticionForm.valid){
      let peticion;
      let importe = parseFloat(this.peticionForm.value.precioUnitario) * this.peticionForm.value.cantidadSuministrto;
